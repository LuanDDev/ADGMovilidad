...
$(document).ready(function () {
    var dsh = {
        ...
        evento() {
            ...
            $(document).on('click', '.cambiarFirma', function () {
                $('#mCambiarFirma').modal('show');
                dsh.getFirma();
            });
            ...
            $(document).on('change', '#fFirma', function () {
                dsh.showimagepreview(this);
            });
        ...
        getFirma() {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (ls.userSignature != undefined && ls.userSignature != null && ls.userSignature != '' ) {
                        document.getElementById("imgPreview").setAttribute("src", '../Signature/' + ls.userSignature);
                    }
                },
                error: function (e) {
        ...
        setFirma() {
            ...
            var fileUpload = $("#fFirma").get(0);
            var files = fileUpload.files;
            objForm.append(files[0].name, files[0]);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: 'Se cambio la firma correctamente',
                        showConfirmButton: false,
                        timer: 2500
                    });
                    setTimeout(() => {
        ...
        GetCompanySession() {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var ls = list.filter(x => x.softwareId == 10);
                    //console.log(ls);
